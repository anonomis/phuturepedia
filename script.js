var Box, Orchestrator, RootWidget, Widget, _, dataResource, j, orchestrator, style,
  ...
  wikipedia: function(page) {
    ...
    j.ajax({
      ...
      success: function(data) {
        return d.resolve(data.parse.text["*"]);
      }
    });
    return d;
  }
...
dataResource.wikipedia("Stockholm").then(function() {});
...
Widget = (function() {
  function Widget() {
    ...
    this.elem = j(this.tmpl).attr("id", this.id);
    this.children = [];
  }
  ...
  Widget.prototype.child = function(widget) {
    this.elem.append(widget.elem);
    return this.children.push(widget);
  };

  Widget.prototype.kill = function() {
    this.clean();
    this.elem.remove();
    return this.ded = true;
  };
  ...
  Widget.prototype.cb = function(fn) {
    console.log("outer", this);
    return (function(_this) {
      return function() {
        console.log("inner", _this);
        if (_this.ded == null) {
          console.log("do", fn, "arg", arguments);
          return fn.apply(_this, arguments);
        } else {
          return console.warn("You are talking to the dead");
        }
      };
    })(this);
  };
...
Box = (function(superClass) {
  ...
  function Box(page) {
    ...
    console.log(this);
    this.getPage("Stockholm");
  }

  Box.prototype.getPage = function(page) {
    return dataResource.wikipedia(page).then(this.cb(this.displayPage));
  };

  Box.prototype.displayPage = function(page) {
    return this.find(".content").html(page);
  };

  Box.prototype.middle = function() {
    return this.find(".sq").css({
      left: "33%",
      right: "33%",
      height: "100%"
    });
  ...
  Box.prototype.upperLeft = function() {
    return this.find(".sq").css({
      left: "0",
      right: "66%",
      height: "33%"
...
Orchestrator = (function(superClass) {
  extend(Orchestrator, superClass);

  function Orchestrator() {
    Orchestrator.__super__.constructor.call(this);
    this.makeBox();
  }

  Orchestrator.prototype.moveUpperLeft = function() {
    return (function(_this) {
      return function() {
        var ref;
        return (ref = _this.children[_this.children.length - 1]) != null ? ref.upperLeft() : void 0;
      };
    })(this);
  };

  Orchestrator.prototype.makeBox = function() {
    var box;
    box = new Box();
    this.child(box);
    return box.middle();
